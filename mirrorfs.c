		    ...
		    struct fuse_file_info *fi)
{
	...
	int i;
	char temp_buf[size];
//Indication that file path being read
...
static int mirror_write(const char *path, const char *buf, size_t size,
		     off_t offset, struct fuse_file_info *fi)
{printf("Running  mirror_write: ");
	int fd;
	int res;
	int i;
	char temp_buf[size];

//Indication that file path being written to
